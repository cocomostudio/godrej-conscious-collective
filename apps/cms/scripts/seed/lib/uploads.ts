...
export async function upload_schedule_document (
	...
	try {
		...
		// Generating the PDF cannot fail in a way worth surviving, but storing
		// it can: with the S3 provider this is a request leaving the machine,
		// and the production host's outbound access is not something the seed
		// gets to assume. An event with no schedule is a shape the frontend
		// already has — the download link is conditional — so this is reported
		// rather than fatal.
		record_failure( {
			what: `The schedule document "${filename}"`,
			where: `Event "${title}" — the Schedule field`,
			why: message( error ),
			how: [
				`This file is generated by the seed, not downloaded, so there `
				+ `is no copy of it to fetch. It is a one-page PDF reading `
				+ `"${title} - schedule".`,
				`Put the real schedule in instead: Content Manager → Event → `
				+ `"${title}" → Schedule → add the PDF, then save and publish.`,
				`Until something is there, the event page renders without its `
...
/**
 ...
 |
 | A slide that will not download is therefore **skipped, and recorded in the
 | run's report** — see `report.ts`, which is where the production host's
 | constrained outbound access is meant to land. A seed with no network at all
 | produces a shell with no slideshow, and
...
async function upload_slide (
	...
	try {
		...
		if ( !response.ok ) {
			report_slide(
				slide,
				`the download answered ${response.status} ${response.statusText}`,
			)
			return null
		}
		...
		report_slide( slide, message( error ) )
		return null
	}
}
...
/**
 |
 | A skipped slide is the seed's most likely failure by a wide margin — it is
 | the only thing here that depends on a host nobody in this project runs, and
 | on the production machine it is the expected outcome rather than the unlucky
 | one. So the report carries everything needed to finish the job without
 | coming back to this file: the address the picture came from, the name it
 | should be stored under, and the two fields an editor has to fill in beside
 | it.
 |
 */
function report_slide ( slide: typeof SLIDESHOW[number], why: string ) {
	record_failure( {
		what: `The registration slideshow picture "${slide.filename}"`,
		where: "Page Shell — the Form slideshow field",
		why,
		how: [
			`Download it from ${slide.url}`,
			`Upload it in Media Library, named ${slide.filename}.`,
			slide.alt === ""
				? `Leave its alternative text empty — this one is decorative.`
				: `Set its alternative text to "${slide.alt}".`,
			slide.caption === undefined
				? `Leave its caption empty; this slide is deliberately without one.`
				: `Set its caption to "${slide.caption}".`,
			`Then add it to Content Manager → Page Shell → Form slideshow, `
			+ `keeping the slides in filename order.`,
		],
	} )
}
